		...
		if (rows.size() > 0)
		{
			...
			for (size_t i = 1; i < rows.size(); i++)
			{
				if (rows[i][0] == '*')
				{
					...
					std::vector<std::string> vals = split_by_cnt(rows[i], '\t', Keys.size());
...
int utils_parse_tsv_file(lua_State*L )
{
	...
	std::string path = FileSystem::GetAbsPath(std::string("resource/tables/") + name);
	std::ifstream fs(path);
	if (!fs)
	{
...
int utils_file_open(lua_State*L)
{
	...
	int mode = luaL_optinteger(L, 2 , std::fstream::in|std::fstream::out);
	...
	fs.open(FileSystem::GetAbsPath(path), mode);
	if (!fs)
		return 0;
