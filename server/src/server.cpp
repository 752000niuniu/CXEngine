...
#define REG_ENUM(name)  (lua_pushinteger(m_L, name),lua_setglobal(m_L, #name))
...
#undef REG_ENUM
	(lua_pushinteger(m_L, CX_MSG_HEADER_LEN), lua_setglobal(m_L, "CX_MSG_HEADER_LEN"));

	script_system_register_function(m_L, net_send_message);
	script_system_register_luac_function(m_L, net_send_message_to_all);
...
int game_server_update(lua_State* L) {
	...
	while (!g_ReadPacketQueue.Empty(NetThreadQueue::Read))
	{
		...
		int32_t type = pt.PeekAsInt32();
		std::string preview(pt.Peek() + 4, pt.readable_size() - 4);
...
int netq_push_login_msg(lua_State*L) {
	...
	buf.Write(msg, strlen(msg));
	q->PushBack(NetThreadQueue::Read, std::string(buf.Peek(), buf.readable_size()));
	return 0;
}
